(function(exports) {
	...
	var WIN32API = (function() {
		...
		var API = {
			...
			// ============ FUNÇÕES EXTRAS PARA GRANNY2.DLL ============
			...
			QueryPerformanceCounter: function(runtime, cpu) {
				...
				var now = performance.now() * 1000000;
				var countLow = now >>> 0;
	...
	fn.stdcall = function(address) {
		...
		// Debug: verificar o endereço
		console.log("stdcall: address=0x" + (address ? address.toString(16) : "undefined/null"));
		console.log("stdcall: num args=" + (arguments.length - 1));
		
		if (!address || address === 0) {
			console.error("stdcall: Invalid address! address=" + address);
		...
		// Ajustar ESP para fazer espaço para return address + argumentos
		// Stack cresce para baixo, então subtraímos
		...
		console.log("stdcall: ESP=0x" + cpu.reg32[reg_esp].toString(16) + ", sp_phys=0x" + sp_phys.toString(16));
		
		// Escrever argumentos primeiro (ordem inversa na stack)
		for(var i = num_args; i >= 1; i--) {
			cpu.memory.mem32s[sp_phys / 4] = arguments[i];
			sp_phys += 4;
		}
		
		// Escrever return address no topo
		cpu.memory.mem32s[sp_phys / 4] = MAGIC_RETURN_ADDR;
		console.log("stdcall: MAGIC_RETURN_ADDR=0x" + MAGIC_RETURN_ADDR.toString(16) + " written at phys 0x" + sp_phys.toString(16));
		
		// Jump to function
		
		cpu.instruction_pointer = address;
		console.log("stdcall: instruction_pointer set to 0x" + cpu.instruction_pointer.toString(16));
	...
	fn.add_hook = function(va, fn) {
		// Criar um nome único para esta função
		var hook_name = "_hook_" + va.toString(16);
		WIN32API[hook_name] = fn;
		this.imports[va] = hook_name;
		console.log("add_hook: registered " + hook_name + " at 0x" + va.toString(16));
	};
